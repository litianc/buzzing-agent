...
// Valid source names
const validSources = ['hn', 'showhn', 'askhn', 'lobsters', 'arstechnica', 'guardian', 'nature', 'skynews', 'devto', 'ph', 'watcha', 'baoyu', 'rundown', 'quesma'] as const;
...
const blogSources = ['baoyu', 'rundown', 'quesma'] as const;
...
async function getSourcePosts(sourceName: string, limit = POSTS_PER_PAGE): Promise<PostCardData[]> {
  try {
    ...
    let result;
    if (isBlogSource) {
      result = await query.orderBy(desc(posts.publishedAt)).limit(limit);
    } else if (isNewsSource) {
      result = await query.orderBy(desc(posts.createdAt)).limit(limit);
    } else {
      result = await query.orderBy(sql`date(${posts.publishedAt}, 'unixepoch') DESC`, desc(posts.score), desc(posts.createdAt)).limit(limit);
    }
