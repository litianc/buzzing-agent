// Buzzing Agent - The Rundown AI Data Fetcher (RSS)
...
const RUNDOWN_RSS_URL = 'https://rss.beehiiv.com/feeds/2R3C6Bt5wj.xml';
...
function extractDomain(url: string): string {
  try {
    ...
    return 'therundown.ai';
  }
}

function generateExternalId(url: string): string {
  // Extract path from beehiiv URL
...
export async function fetchRundown(options: { limit?: number } = {}): Promise<{ count: number; newPosts: number; deleted: number; duration: number }> {
  const startTime = Date.now();
  const { limit = 30 } = options;

  try {
    let source = await db.query.sources.findFirst({ where: eq(sources.name, 'rundown') });
    if (!source) {
      const [created] = await db.insert(sources).values({
        name: 'rundown',
        displayName: 'The Rundown AI',
        description: 'Daily AI news summary covering the latest developments in artificial intelligence',
        apiEndpoint: RUNDOWN_RSS_URL,
        minScore: 0,
    ...
    const feed = await parser.parseURL(RUNDOWN_RSS_URL);
    ...
    await db.insert(fetchLogs).values({ sourceName: 'rundown', status: 'success', itemsCount: newPostsCount, duration });
    return { count: items.length, newPosts: newPostsCount, deleted: deletedCount, duration };
  } catch (error) {
    const duration = Date.now() - startTime;
    await db.insert(fetchLogs).values({ sourceName: 'rundown', status: 'failed', itemsCount: 0, errorMsg: error instanceof Error ? error.message : 'Unknown', duration });
    throw error;
  }
}
