// Buzzing Agent - Show HN Data Fetcher (reuses HN API)
...
export async function fetchShowHN(options: { limit?: number; minScore?: number; translate?: boolean } = {}): Promise<{ count: number; newPosts: number; deleted: number; duration: number }> {
  const startTime = Date.now();
  const { limit = 30, minScore = 10, translate = true } = options;

  try {
    let source = await db.query.sources.findFirst({ where: eq(sources.name, 'showhn') });
    if (!source) {
      const [created] = await db.insert(sources).values({
        name: 'showhn', displayName: 'Show HN', description: 'Hacker News 展示区，开发者分享自己的项目',
    ...
    for (const item of validStories) {
      ...
      const postData: NewPost = {
        ...
        sourceUrl, originUrl: `https://news.ycombinator.com/item?id=${item.id}`,
        sourceDomain: extractDomain(sourceUrl), author: item.by || null,
        authorUrl: item.by ? `https://news.ycombinator.com/user?id=${item.by}` : null,
        score: item.score || 0, tags: ['Show HN'], publishedAt: truncateToMinute(item.time * 1000),
      };

      if (translate) {
        try {
          const translations = await translatePostToAllLocales({ titleOriginal: postData.titleOriginal!, originalLang: 'en' });
    ...
    await db.insert(fetchLogs).values({ sourceName: 'showhn', status: 'success', itemsCount: newPostsCount, duration });
    return { count: validStories.length, newPosts: newPostsCount, deleted: deletedCount, duration };
  } catch (error) {
    const duration = Date.now() - startTime;
    await db.insert(fetchLogs).values({ sourceName: 'showhn', status: 'failed', itemsCount: 0, errorMsg: error instanceof Error ? error.message : 'Unknown', duration });
    throw error;
  }
}
